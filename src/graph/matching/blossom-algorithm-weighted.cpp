struct weighted_blossom {
	...
	int lab[MAXN * 2 + 1], match[MAXN * 2 + 1], slack[MAXN * 2 + 1], st[MAXN * 2 + 1], pa[MAXN * 2 + 1];
	int flower_from[MAXN * 2 + 1][MAXN + 1], S[MAXN * 2 + 1], vis[MAXN * 2 + 1];
	std::vector <int> flower[MAXN * 2 + 1]; std::queue <int> q;
	...
	void set_slack (int x) { slack[x] = 0; for (int u = 1; u <= n; ++u) if(g[u][x].w > 0 && st[u] != x && S[st[u]] == 0)
		update_slack(u, x); }
	void q_push (int x) {
		if (x <= n) q.push (x);
		else for (size_t i = 0; i < flower[x].size (); i++) q_push (flower[x][i]); }
	void set_st (int x, int b) {
		st[x] = b; if (x > n) for (size_t i = 0; i < flower[x].size (); ++i) set_st (flower[x][i], b); }
	int get_pr (int b, int xr){
		int pr = std::find (flower[b].begin (), flower[b].end (), xr) - flower[b].begin ();
		if (pr % 2 == 1) { std::reverse (flower[b].begin () + 1, flower[b].end ()); return (int) flower[b].size () - pr;
		} else return pr; }
	void set_match (int u, int v){
		match[u] = g[u][v].v; if (u > n) {
			edge e = g[u][v]; int xr = flower_from[u][e.u], pr = get_pr (u, xr);
			for (int i = 0; i < pr; ++i) set_match (flower[u][i], flower[u][i ^ 1]);
			set_match (xr, v); std::rotate (flower[u].begin (), flower[u].begin () + pr, flower[u].end ()); } }
	void augment (int u, int v) {
		for (; ; ) {
			int xnv = st[match[u]]; set_match (u, v);
			if (!xnv) return; set_match (xnv, st[pa[xnv]]);
			u = st[pa[xnv]], v = xnv; } }
	...
	void add_blossom (int u, int lca, int v) {
		...
		if (b > n_x) ++n_x;
		lab[b] = 0, S[b] = 0;
		match[b] = match[lca]; flower[b].clear ();
	...
	void expand_blossom (int b) { 
		...
		for (size_t i = pr + 1; i < flower[b].size (); ++i) {
			int xs = flower[b][i]; S[xs] = -1, set_slack(xs); }
		st[b] = 0; }
	...
	bool matching () {
		...
		for (; ; ) {
			...
			int d = INF;
			for (int b = n + 1; b <= n_x; ++b) if(st[b] == b && S[b] == 1) d = std::min (d, lab[b] / 2);
	...
	std::pair <long long, int> solve () {
		...
		int n_matches = 0; long long tot_weight = 0;
		for (int u = 0; u <= n; ++u) st[u] = u, flower[u].clear();
		int w_max = 0;
		for (int u = 1; u <= n; ++u) for (int v = 1; v <= n; ++v) {
			flower_from[u][v] = (u == v ? u : 0); w_max = std::max (w_max, g[u][v].w); }
		for (int u = 1; u <= n; ++u) lab[u] = w_max;
		while (matching ()) ++n_matches;
		for (int u = 1; u <= n; ++u) if (match[u] && match[u] < u) tot_weight += g[u][match[u]].w;
		return std::make_pair (tot_weight, n_matches); }
	void init () { for (int u = 1; u <= n; ++u) for (int v = 1; v <= n; ++v) g[u][v] = edge (u, v, 0); } };
