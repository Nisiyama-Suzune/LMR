...
//		for (int i = 0; i < k; i++) ret = std::max (ret, std::abs (a.data[i] - b.data[i]));
		return ret; }
...
kd_tree <> _k;
kd_tree <>::point pp[200000];

int main () {
	int T; std::cin >> T; while (T--) {
		int N; std::cin >> N;
		for (int i = 0; i < N; ++i) { std::cin >> pp[i].data[0] >> pp[i].data[1]; _k.p[i] = pp[i]; _k.p[i].id = i; }
		_k.init (N, 2);
		for (int i = 0; i < N; ++i) {
			std::cout << _k.min_kth (pp[i], 2).dist << "\n";
		}
	}
	return 0;
}
